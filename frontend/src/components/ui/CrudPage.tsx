...
type Field={name:string;label:string;type?:string;required?:boolean;emptyValue?:unknown;options?:{label:string;value:string|number}[]|((rows:any[],editing:any|null)=>{label:string;value:string|number}[])};
type Props={
...
export function CrudPage({title,subtitle,queryKey,api,columns,fields,tenantId}:Props){
  ...
  const submit=(e:FormEvent)=>{e.preventDefault();const data=Object.fromEntries(fields.flatMap(f=>form[f.name]===undefined?[]:form[f.name]===''&&f.emptyValue===undefined?[]:[[f.name,form[f.name]===''?f.emptyValue:form[f.name]]]));if(tenantId && !('tenantId' in data))data.tenantId=tenantId;mutation.mutate(data)};
  return <div>
