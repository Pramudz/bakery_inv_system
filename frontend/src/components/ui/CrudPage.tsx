...
type Props = {
  ...
  api: {
    ...
    update?: (id: number, x: any) => Promise<any>;
...
export function CrudPage({
  ...
  const mutation = useMutation({
    mutationFn: (x: any) =>
      editing && api.update ? api.update(editing.id, x) : api.create(x),
    onSuccess: () => {
      ...
      fields.flatMap((f) =>
        form[f.name] === undefined
          ...
          : form[f.name] === "" && f.emptyValue === undefined
            ? []
            : [[f.name, form[f.name] === "" ? f.emptyValue : form[f.name]]],
      ),
    );
    if (tenantId && !("tenantId" in data)) data.tenantId = tenantId;
        ...
        <table className="table">
          ...
          <tbody>
            {q.isLoading ? (
              ...
              <tr>
                <td colSpan={columns.length + 1}>Unable to load data</td>
              </tr>
            ) : (
              (q.data ?? []).map((r: any) => (
                <tr key={r.id}>
                  {columns.map((c) => (
                    <td key={c.key}>
                      {typeof r[c.key] === "boolean" ? (
                        <span
                          className={
                            r[c.key] ? "badge badge-on" : "badge badge-off"
                          }
                        >
                          {r[c.key] ? "Active" : "Inactive"}
                        </span>
                      ) : (
                        String(
                          (c.key === "id"
