...
test('formats first SKU, PO, GRN, adjustment and conversion numbers', () => {
...
test('failed Product, PO, GRN, adjustment and conversion transactions do not commit counter increments', async () => {
  const service = new NumberSequencesService();
  for (const [sequenceKey, periodKey] of [
    ...
    [NumberSequenceKeys.INVENTORY_CONVERSION, '2026'],
  ] as const) {
