import { ConflictException, Injectable, NotFoundException } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { Brand } from './brands.entity';
import { CreateBrandDto } from './dto/create-brands.dto';
import { UpdateBrandDto } from './dto/update-brands.dto';

@Injectable()
export class BrandService {
  constructor(@InjectRepository(Brand) private readonly repo: Repository<Brand>) {}

  findAll(tenantId: number) {
    return this.repo.find({
      where: { tenantId },
      order: { brandId: 'ASC' },
    });
  }

  async findPage(
    tenantId: number,
    page: number,
    limit: number,
    search: string,
    status: string,
  ) {
    const safePage = Math.max(1, Number.isFinite(page) ? page : 1);
    const safeLimit = [20, 50, 100].includes(limit) ? limit : 20;
    const searchText = search.trim();
    const query = this.repo
      .createQueryBuilder('brand')
      .leftJoinAndSelect('brand.tenant', 'tenant')
      .where('brand.tenantId = :tenantId', { tenantId });

    if (searchText) {
      query.andWhere(
        '(LOWER(brand.brandCode) LIKE LOWER(:search) OR LOWER(brand.brandName) LIKE LOWER(:search))',
        { search: `%${searchText}%` },
      );
    }
    if (status === 'active') {
      query.andWhere('brand.isActive = :active', { active: true });
    }
    if (status === 'inactive') {
      query.andWhere('brand.isActive = :active', { active: false });
    }

    const [rows, total] = await query
      .orderBy('brand.brandName', 'ASC')
      .addOrderBy('brand.brandId', 'ASC')
      .skip((safePage - 1) * safeLimit)
      .take(safeLimit)
      .getManyAndCount();
    const items = rows.map((row) => {
      const { tenantId: _tenantId, ...item } = row;
      return item;
    });
    return {
      items,
      page: safePage,
      limit: safeLimit,
      total,
      totalPages: Math.max(1, Math.ceil(total / safeLimit)),
    };
  }

  async findOne(id: number, tenantId: number) {
    const row = await this.repo.findOne({
      where: { brandId: id, tenantId } as any,
    });
    if (!row) throw new NotFoundException('Brand not found');
    return row;
  }

  async create(dto: CreateBrandDto, tenantId: number) {
    const payload: any = { ...dto, tenantId };
    const existing = await this.repo.findOne({
      where: {
        tenantId,
        brandCode: (dto as any).brandCode,
      } as any,
    });
    if (existing) throw new ConflictException('Code already exists for this tenant.');
    return this.repo.save(this.repo.create(payload));
  }

  async update(id: number, dto: UpdateBrandDto, tenantId: number) {
    await this.findOne(id, tenantId);
    const payload: any = { ...dto };
    delete payload.tenantId;
    if (payload.brandCode) {
      const same = await this.repo.findOne({
        where: { tenantId, brandCode: payload.brandCode } as any,
      });
      if (same && (same as any).brandId !== id) {
        throw new ConflictException('Code already exists for this tenant.');
      }
    }
    await this.repo.update({ brandId: id, tenantId } as any, payload);
    return this.findOne(id, tenantId);
  }

  async deactivate(id: number, tenantId: number) {
    await this.findOne(id, tenantId);
    await this.repo.update({ brandId: id, tenantId } as any, { isActive: false } as any);
    return this.findOne(id, tenantId);
  }

  async activate(id: number, tenantId: number) {
    await this.findOne(id, tenantId);
    await this.repo.update({ brandId: id, tenantId } as any, { isActive: true } as any);
    return this.findOne(id, tenantId);
  }
}
