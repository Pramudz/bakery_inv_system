...
export class AttributeService {
  ...
  async findOne(id: number, tenantId: number) {
    const row = await this.repo.findOne({
      where: { attributeId: id, tenantId } as any,
    });
    if (!row) throw new NotFoundException('Attribute not found');
    return row;
  }

  async create(dto: CreateAttributeDto, tenantId: number) {
  ...
  async update(id: number, dto: UpdateAttributeDto, tenantId: number) {
    ...
    if (payload.code) {
      ...
      if (same && (same as any).attributeId !== id) {
        throw new ConflictException('Code already exists for this tenant.');
      }
    }
    await this.repo.update({ attributeId: id, tenantId } as any, payload);
    return this.findOne(id, tenantId);
  }

  async deactivate(id: number, tenantId: number) {
    await this.findOne(id, tenantId);
    await this.repo.update({ attributeId: id, tenantId } as any, { isActive: false } as any);
    return this.findOne(id, tenantId);
  }
}
